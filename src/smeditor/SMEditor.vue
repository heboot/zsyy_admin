<template>
    <div class="smeditor" id="smeditor">
        <div class="buttons" :class="buttonsBarFixed == true ? 'isFixed' :''">
            ...
            <button type="button" class='remove-format' title="清除" @click='removeFormat'
            ...
            <button type="button" class="font-size" title="字号" @click="isFontSizePickerShow = !isFontSizePickerShow"
                v-on:mouseover.stop='mouseover($event)'>
                <span> {{fontSize}} </span>
                <font-size-picker v-bind:FontSizePickerClick="fontSizePickerClick" v-show="isFontSizePickerShow">
                </font-size-picker>
            </button>
            <button type="button" v-for='(name, index) in basicIcons' @click='basicStyleClick(name.name)'
                v-bind:class="{buttonsActive: styles.indexOf(name.name) > -1}" v-on:mouseover.stop='mouseover($event)'
            ...
            <button type="button" class='indent' @click.stop='indent' v-on:mouseover.stop='mouseover($event)'
                title="增加缩进">
                <img src='../images/rightindent.png'></img>
            </button>
            <button type="button" class='outdent' @click.stop='outdent' v-on:mouseover.stop='mouseover($event)'
                title="减少缩进">
      ...
      <button type="button" class='insert-ul' @click='insertList("UnorderedList")' v-on:mouseover.stop='mouseover($event)' title="无序列表">
        <img src='../images/unorder.png'></img>
            ...
            <button type="button" class='align-left' @click='align("Left")' v-on:mouseover.stop='mouseover($event)'
                title="左对齐">
                <img src='../images/left.png'></img>
            </button>
            <button type="button" class='align-center' @click='align("Center")' v-on:mouseover.stop='mouseover($event)'
                title="居中对齐">
                <img src='../images/center.png'></img>
            </button>
            <button type="button" class='align-right' @click='align("Right")' v-on:mouseover.stop='mouseover($event)'
                title='右对齐'>
      ...
      <button type="button" class='insert-link' @click='insertLinkClick'  v-on:mouseover.stop='mouseover($event)' title='插入链接'>
        <img :src='icons.insertLink'></img>
            ...
            <button type="button" class='insert-options' @click="isInsertShow = !isInsertShow">
                ...
                <insert-options v-show="isInsertShow" :insertImage="insertImageClick" :insertLine="insertLine"
                    :uploadImages='uploadImages' :insertVideo="insertVideoClick" :insertBlock="insertBlock">
                </insert-options>
            </button>
            <button type="button" class="backup" @click='backupClick' v-on:mouseover.stop='mouseover($event)'
                title="Ctrl + S"></button>
        ...
        <div contenteditable="true" autocorrect="off" autocomplete="off" spellcheck="false" class="input-area"
            id="input-area" v-on:mouseup="mouseup" v-on:keyup.enter="kenter" v-on:keyup.ctrl.83="backupClick"
            v-on:keyup.ctrl.80="previewClick">
        </div>
        <p class="select-words" v-show="selectWords">{{selectWords.length}}个字</p>
        <insert-link :insertLink='insertLink' :propText='insertLinkSection.text' :propLink='insertLinkSection.link'
            v-show='isInsertLinkShow' :cancel='insertLinkCancel'></insert-link>
    ...
    data() {
        return {
            ...
            basicIcons: [
                ...
                {
                    name: "strikethrough",
                    img: require("../images/strikethrough.png")
                }
            ],
    ...
    methods: {
        ...
        titlePickerClick(size, index) {
            ...
            if (node.localName.startsWith("h") && size === "正文") {
                ...
                node.localName.startsWith("h") === false &&
                size !== "正文"
            ) {
                html = `<${size}>${node.innerHTML}</${size}>`;
            } else {
        ...
        insertQuote() {
            ...
            if (
                ...
                node.parentNode.className !== "input-area"
            ) {
                ...
                document.execCommand(
                    "insertHTML",
                    false,
                    `<div class="blockquote"><blockquote style="color: #B2B2B2; padding-left: 15px; border-left: 5px solid #B2B2B2; margin-top: 0px; margin-bottom: 0px;">${node.innerHTML}</blockquote></div>`
                );
                node.outerHTML = "";
            } else {
                document.execCommand(
                    "insertHTML",
                    false,
                    `<div class="blockquote"><blockquote style="color: #B2B2B2; padding-left: 15px; border-left: 5px solid #B2B2B2; margin-top: 0px; margin-bottom: 0px;"><span><br></span></blockquote></div>`
                );
            }
        },
        // 插入 有序/无序 列表
      ...
      `
        ...
        restoreClick() {
            editorElement().innerHTML =
                window.localStorage.getItem("smeditor") || "";
        },
        // 预览预览
        previewClick() {
...
.smeditor .select-words {
    ...
    -webkit-box-shadow: 0 2px 8px hsla(0, 0%, 70%, 0.8);
    -moz-box-shadow: 0 2px 8px hsla(0, 0%, 70%, 0.8);
    -ms-box-shadow: 0 2px 8px hsla(0, 0%, 70%, 0.8);
    -o-box-shadow: 0 2px 8px hsla(0, 0%, 70%, 0.8);
    box-shadow: 0 2px 8px hsla(0, 0%, 70%, 0.8);
