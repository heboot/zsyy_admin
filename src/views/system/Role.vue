<template>
    <div>
        <Card>
            ...
            <Modal v-model="delRole" title="删除角色" @on-ok="deleteRole" @on-cancel="$Message.info('已取消！')">
                <p>是否删除该角色?</p>
            </Modal>
            <Modal v-model="showAddRole" title="添加角色" @on-ok="addRole">
                <Form label-position="right" :model="formValidate" :label-width="100" :rules="ruleValidate">
                    <FormItem label="角色名称：" prop="name">
                        <Input v-model="formValidate.name" :maxlength="10"></Input>
                    </FormItem>
                    <FormItem label="角色描述：">
                        <Input type="textarea" v-model="formValidate.roleDscrp" :maxlength="200"></Input>
                    </FormItem>
                </Form>
                <br />
            ...
            <Modal v-model="showForm" title="修改权限" @on-ok="confirmChange" @on-cancel="cancelChange">
                <Form>
                    <FormItem label="角色描述：">
                        <Input type="textarea" v-model="formValidate.roleDscrp" :maxlength="200"></Input>
                    </FormItem>
            ...
            <Row class="pageWrapper">
                <Page :total="pageNum" class="buttomPage" @on-change="changePage" :current="currentPageIdx"
                    show-elevator></Page>
            </Row>
        </Card>
...
export default {
                                    ...
                                    on: {
                                        click: () => {
                                            ...
                                            this.formValidate.name =
                                                obj.row.role;
                                            this.formValidate.roleDscrp =
                                                obj.row.description;
                                            let initTree = JSON.parse(
                                                getStore("allPermission")
                                            );
    ...
    methods: {
        ...
        deleteRole() {
            deleteRole({ id: this.deleteRoleID }).then(res => {
                if (!res.code) {
        ...
        refreshPageManual() {
            this.currentPageIdx = 1;
            this.refreshPage({ pageSize: this.pageSize, pageNo: this.pageNo });
        },
        refreshPage(obj) {
            this.ifLoading = true;
            roleManage(obj).then(
