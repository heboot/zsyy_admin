<template>
    <div>
        <Card>
            ...
            <Row>
                <Table :columns="columns_first" :data="userpage" border :loading="ifLoading"
                    @on-selection-change="selected" class='user_center'>
                </Table>
            </Row>

            <Row class="pageWrapper">
                <Page :total="totalNum" :current="current" @on-change="changePage" show-elevator>
                </Page>
            </Row>

            <Modal class="auditPermissionModal" v-model="ifAudit" title="编辑权限" @on-ok="confirmAudit">
                <ul>
                    <li>
                        <span><i>*</i>权限标题：</span>
                        <p>
                    ...
                    <li>
                        ...
                        <p>
                            <Select v-model="parentID">
                                ...
                                <Option v-for="item in selectArr" :value="item.id" :key="item.id">{{ item.title }}
                                </Option>
                            </Select>
                        </p>
                    </li>
            ...
            <Modal class="auditPermissionModal" v-model="ifDelete" title="删除权限" @on-ok="confirmDel">
    ...
    methods: {
        ...
        confirmAudit() {
            if (!this.title || !this.name || !this.parentID) {
                ...
                addAuditPermission(obj).then(res => {
                    if (!res.code) {
        ...
        addPermissionBtn() {
            ...
            this.ifAudit = true;
            permissionManage({ parentId: 0 }).then(res => {
                console.log(res);
